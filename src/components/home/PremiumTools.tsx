...
import { ExternalLink, Sparkles, Image as ImageIcon, MessageSquare, Wand2, ArrowRight, Crown } from "lucide-react";
import { Link } from "react-router-dom";
import { useState, useEffect } from "react";
import { collection, query, onSnapshot } from "firebase/firestore";
import { db } from "../../lib/firebase";

const STATIC_PREMIUM_TOOLS = [
  {
    id: "static-1",
    title: "MIR GPT",
    shortTitle: "GPT",
...
export function PremiumTools() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snapshot) => {
      const tools = snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
        isAppRoute: false // they either go to CustomToolViewer or external link
      }));
  ...
  const allTools = [...STATIC_PREMIUM_TOOLS, ...dynamicTools];
  ...
  return (
    <section className="py-32 relative">
      ...
      <div className="container mx-auto max-w-7xl px-4 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 flex-wrap">
          {allTools.map((tool, idx) => {
            const isLarge = tool.spanClass?.includes("col-span-8") || tool.spanClass?.includes("col-span-12");
            const href = tool.isAppRoute ? tool.url : (tool.htmlCode ? `/custom-tool/${tool.id}` : tool.url);
            const isExternal = !tool.isAppRoute && tool.url && !tool.htmlCode;
            
            const ToolWrapper = ({ children }: { children: React.ReactNode }) => {
              if (isExternal) {
                 return <a href={href} target="_blank" rel="noreferrer" className="absolute inset-0 p-[2px] rounded-[2.5rem] overflow-hidden">{children}</a>
              }
              return <Link to={href} className="absolute inset-0 p-[2px] rounded-[2.5rem] overflow-hidden">{children}</Link>
            };
            ...
            return (
              <motion.div
                ...
                className={`${tool.spanClass || 'col-span-1 md:col-span-6 lg:col-span-4'} group relative h-[320px] md:h-[380px] rounded-[2.5rem] overflow-hidden`}
              >
                {/* Glow effect */}
                <div className={`absolute inset-0 bg-gradient-to-br ${tool.color || 'from-white/10 to-white/5'} opacity-0 group-hover:opacity-10 transition-opacity duration-500`}></div>
                ...
                <ToolWrapper>
                  ...
                  <div className="w-full h-full bg-[#0a0a0f] rounded-[calc(2.5rem-2px)] p-8 relative overflow-hidden flex flex-col justify-between border border-white/5">
                    ...
                    <div className="absolute right-0 top-0 -mr-20 -mt-20 w-64 h-64 opacity-[0.03] text-white pointer-events-none">
                       {tool.icon ? (
                    ...
                    <div className="relative z-10 flex justify-between items-start">
                      <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${tool.color} p-0.5 shadow-lg ${tool.shadow || ''}`}>
                        <div className="w-full h-full bg-[#0a0a0f] rounded-2xl flex items-center justify-center p-2">
                           {tool.iconUrl ? (
                              <img src={tool.iconUrl} className="w-full h-full object-contain rounded-xl" />
                           ) : tool.icon ? (
                              <tool.icon className={`w-8 h-8 text-transparent bg-clip-text bg-gradient-to-br ${tool.color}`} />
                           ) : (
                              <Sparkles className={`w-8 h-8 text-transparent bg-clip-text bg-gradient-to-br ${tool.color}`} />
                           )}
                        </div>
                      </div>
                    ...
                    <div className="relative z-10 mt-auto">
                      ...
                      <div className="mt-8 flex items-center gap-2 text-primary font-medium opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300">
                        Launch Application {isExternal ? <ExternalLink className="w-4 h-4 ml-1" /> : <ArrowRight className="w-4 h-4 ml-1" />}
                      </div>
                    </div>
